     ...
     .controller('CustomerEnrollmentCtrl', [
        ...
        function ($scope, $http, $templateCache, $compile, $ceFactory, $beneficiaryService, $ceService, $identificationService, $timeout, $filter, AngularThinBox) {
            ...
            $scope.open = function (id, tabId) {
        		...
        		$scope.allowBene = $scope.nceobj.ShowBeneficiariesTab && $scope.serviceId === 111;
            ...
            $scope.getRequiredFields = function (sync, id) {
                $http({
                    ...
                    if (id > 0) {
                        var validationData = parent.$("#unifyingDiv").data("orderValidationData");
            ...
            $scope.save = function (actionId) {
                ...
                $http({
                    ...
                    if (data.result.HasDuplicate) {
                        ...
                        else if (data.requireIdentification) {
                        	$scope.getCustomerIds();
                        }
            ...
            $scope.backToIdentificationsPage = function () {
                $('#loader', self.parent.document).hide();
                $('#container', self.parent.document).show();
                $("#divConfirmUpload").hide();
                $("#anchorContainer", self.parent.document).show();
                $("#imageList", self.parent.document).show();
                $("#iframeUploadFile", self.parent.document).hide();
            };

            $scope.resetAfterFileUpload = function(data) {
                if (data.imageId !== -1) {

                    $scope.i.ImageId = data.imageId; // value is -1 if there is no images until successful upload/scan

                    $scope.displayCarouselImages($scope.i.Id, data.imageId);

                    // hide upload form
                    $scope.backToIdentificationsPage();
            ...
            $("#formUpload").submit(function () {
                ...
                {
                    var file = document.getElementById("inputFileUpload");

                    var formData = new FormData();

                    formData.append('custId', $scope.c.Id);
                    formData.append('identId', $scope.i.Id);
                    formData.append('imageId', $scope.i.ImageId == null || $scope.i.ImageId == undefined ? 0 : $scope.i.ImageId);
                    ...
                    $http({
                            method: 'POST',
                            data: formData,
                            url: '/Classic/CustomerEnrollment2/UploadFile',
                        ...
                        .success(function (data) {
                           ...
                           $scope.resetAfterFileUpload(data);

                        });

                    return false; // pleaes keep this so form does not get submitted twice
            ...
            $scope.changeFileSelection = function (e) {
                var inputFileDOM = document.getElementById("inputFileUpload");
                var files = inputFileDOM.files;
                var filePath = inputFileDOM.value;
                var fileName = !!files ? files[0].name : filePath.substring(filePath.lastIndexOf("\\") + 1);
                var fileSize = !!files ? files[0].size : 1; //for IE < Edge the file size property is not accessible
                var fileExtension = fileName.substring(fileName.lastIndexOf(".") + 1);

                if (inputFileDOM.accept.indexOf(fileExtension.toUpperCase()) < 0) {
                    if (e.stopPropagation) e.stopPropagation();
                    else e.cancelBubble = true;
                    alert(Resources.Client.get("fileNotAllowed"));
                    return;
                }
                if (eval(inputFileDOM.max_size) < fileSize) {
                    if (e.stopPropagation) e.stopPropagation();
                    else e.cancelBubble = true;
                    alert(Resources.Client.get("fileSizeExceedsLimitWarning") + " " + document.getElementById("fileSizeLimit").value);
                    return;
                }
                var $spanConfirmUpload = $("#spanConfirmUpload");
                $spanConfirmUpload.html(Resources.Client.get("confirmUpload").format(filePath));
                $("#anchorContainer", self.parent.document).hide();
                $("#imageList", self.parent.document).hide();
                $("#iframeUploadFile", self.parent.document).show();
                $("#divConfirmUpload").show();
            };

            $scope.startBrowseFiles = function () {
                var $iframeUploadFile = $("#iframeUploadFile");
                var $inputFileUpload = $iframeUploadFile.contents().find("#inputFileUpload");

                $inputFileUpload.closest("form").trigger("reset");
                $inputFileUpload.click();               
            };

            $scope.backToIdentificationsPage(); //needed in initial load to reset file upload items

            //hide file upload functionality for old browsers  (ie9 and below)
            if (window.FormData === undefined) {
                $timeout(function () { $scope.allowUploadImageFromDisk = false; }, 500);
            }
                ...
                $beneficiaryService.getBeneficiaries($scope.c.Id).then(function (data) {
                    ...
                    if ($scope.savedBen) {
                        ...
                        if ($scope.savedBen.BeneId > 0 && benObj) {
                            $scope.selectBeneficiary(benObj[0]); 
                        }
